...
def run_vwap_bounce():
    ...
    vwap_calc AS (
        SELECT 
            *,
            SUM(tp * volume) OVER (PARTITION BY symbol, date_ist ORDER BY timestamp) as cum_tp_vol,
            SUM(volume) OVER (PARTITION BY symbol, date_ist ORDER BY timestamp) as cum_vol,
    ...
    vwap_final AS (
        SELECT 
            *,
            (cum_tp_vol / NULLIF(cum_vol, 0)) as vwap
        FROM vwap_calc
    ),
    signals AS (
        SELECT 
            *,
            -- LONG: Trend is UP (vwap rising or close > vwap), touched vwap, closed green, high volume
            CASE WHEN low < vwap AND close > vwap AND close > open AND volume > sma_vol_20 * 2.0 AND time_ist >= '09:30' AND time_ist <= '14:00' THEN 1 ELSE 0 END as signal_long,
            -- SHORT: Touched vwap from below, closed red, high volume
            CASE WHEN high > vwap AND close < vwap AND close < open AND volume > sma_vol_20 * 2.0 AND time_ist >= '09:30' AND time_ist <= '14:00' THEN 1 ELSE 0 END as signal_short
        FROM vwap_final
    )
    ...
    for i, row in df.iterrows():
        ...
        stop = entry_price * 0.990 if side == 'LONG' else entry_price * 1.010
