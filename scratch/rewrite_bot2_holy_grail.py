...
mcx_path = 'services/backtest_mcx_service.py'
with open(mcx_path, 'r', encoding='utf-8') as f:
    mcx_content = f.read()

parts = mcx_content.split('def run_bot2_mcx_backtest(params: dict) -> tuple[bool, dict, int]:')
if len(parts) == 2:
    top = parts[0]
    
    bot2_mcx = """def run_bot2_mcx_backtest(params: dict) -> tuple[bool, dict, int]:
    try:
...
"""
    with open(mcx_path, 'w', encoding='utf-8') as f:
        f.write(top + bot2_mcx)
    print("Updated MCX backend for Holy Grail Bot 2.")

# ------------- NSE ----------------

nse_path = 'services/backtest_service.py'
with open(nse_path, 'r', encoding='utf-8') as f:
    nse_content = f.read()

parts2 = nse_content.split('def run_bot2_backtest(params: dict) -> tuple[bool, dict, int]:')
if len(parts2) == 2:
    top2 = parts2[0]
    
    bot2_nse = """def run_bot2_backtest(params: dict) -> tuple[bool, dict, int]:
    try:
        from strategies.scripts.bot2_ema_momentum import EMAMomentum, check_signals
        from datetime import time as time_obj
        from database.historify_db import get_ohlcv
        ...
        # NSE lot size multiplier handles units directly in the service
        # BANKNIFTY is usually 15
        unit_size = 15 if symbol == "BANKNIFTY" else 25 if symbol == "NIFTY" else 1
        ...
        return False, {"status": "error", "message": str(e)}, 500
"""
    with open(nse_path, 'w', encoding='utf-8') as f:
        f.write(top2 + bot2_nse)
    print("Updated NSE backend for Holy Grail Bot 2.")
else:
    print("Could not find run_bot2_backtest in NSE service!")
