class VolrixBot4(Strategy):
    ...
    def onCandleClose(self):
        ...
        if not self.morning_entry_done and (not self.aborted_for_day) and (self.candleTime.hour >= 9 and self.candleTime.minute >= 30):
            self.morning_entry_done = True
            self.sl_targets['morning_CE'] = spot_close * 1.01
            self.sl_targets['morning_PE'] = spot_close * 0.99
            for opt in ('CE', 'PE'):
                leg = self.add_managed_leg(side='sell', option_type=opt, lots=1, strike_selection={'strikeBy': 'moneyness', 'strikeVal': 0, 'asof': 'None', 'roundoff': None}, exp={'expType': 'weekly', 'expNo': 0}, stop_loss={'isSL': False, 'SLon': '%', 'SLvalue': 0}, target={'isTarget': False, 'targetOn': 'val', 'targetValue': 0}, trailing_stop_loss={'isTrailSL': False, 'trailSLon': 'val', 'trailSL_X': 0, 'trailSL_Y': 0}, stop_loss_reentry={'isReEntry': False, 'reEntryOn': 'asap', 'reEntryVal': 0, 'reEntryMaxNo': 0}, target_reentry={'isReEntry': False, 'reEntryOn': 'asap', 'reEntryVal': 0, 'reEntryMaxNo': 0}, wait_trade={'isWT': False, 'wtOn': 'val-up', 'wtVal': 0, 'triggers': []}, segment='OPT', square_off='this', leg_name=f'morning_{opt}', remark=f'09:30 ATM {opt}')
        ...
        if self.morning_entry_done and (not self.recovery_done) and (not self.aborted_for_day):
            ...
            if not has_ce and has_pe:
                if self.rolls_done < self.max_rolls:
                    ...
                    leg = self.add_managed_leg(side='sell', option_type='PE', lots=1, strike_selection={'strikeBy': 'moneyness', 'strikeVal': 0, 'asof': 'None', 'roundoff': None}, exp={'expType': 'weekly', 'expNo': 0}, stop_loss={'isSL': False}, target={'isTarget': False}, trailing_stop_loss={'isTrailSL': False}, stop_loss_reentry={'isReEntry': False}, target_reentry={'isReEntry': False}, wait_trade={'isWT': False}, segment='OPT', square_off='this', leg_name='roll_PE', remark='Roll PE to ATM')
                ...
                if self.rolls_done < self.max_rolls:
                    ...
                    leg = self.add_managed_leg(side='sell', option_type='CE', lots=1, strike_selection={'strikeBy': 'moneyness', 'strikeVal': 0, 'asof': 'None', 'roundoff': None}, exp={'expType': 'weekly', 'expNo': 0}, stop_loss={'isSL': False}, target={'isTarget': False}, trailing_stop_loss={'isTrailSL': False}, stop_loss_reentry={'isReEntry': False}, target_reentry={'isReEntry': False}, wait_trade={'isWT': False}, segment='OPT', square_off='this', leg_name='roll_CE', remark='Roll CE to ATM')
                    self.actions_all['act_roll']['legs'].append(leg)
                    self.rolls_done += 1
