class Volrixbot5(Strategy):
    ...
    def minTrigger(self):
        import datetime
        # Hard square off at 23:15 for MCX
        if self.candleTime >= datetime.time(23, 15):
            self.square_off_all_positions(remark="EOD Square Off")
            self.stop_backtest()
            return

        if not self.aborted_for_day or self.recovery_done:
            # 22:00 Timed Exit for Recovery
            if self.recovery_done and not self.recovery_timed_exit_done and self.candleTime >= datetime.time(22, 0):
                if self.open_mtm < 0:
    ...
    def _get_base_lot_size(self):
        ...
        if s == "SENSEX": return 10
        if s == "FINNIFTY": return 40
    ...
    def _get_dynamic_capital(self):
        # We will assume a flat baseline margin multiplier based on the lot size ratio.
        # For base testing on MCX, we assume 1.85L per standard lot.
