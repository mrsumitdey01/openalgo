...
==============================
Bot6 Multi-Asset Scanner — Main Entry Point.
...
LOGGING:
  Default: INFO level to stdout + bot6_scanner.log file
...
# ---------------------------------------------------------------------------
# Path setup: add openalgo root so `bot6` can be imported
# ---------------------------------------------------------------------------
...
def setup_logging(debug: bool = False) -> None:
    ...
    handlers = [
        ...
        logging.FileHandler("bot6_scanner.log", mode="a", encoding="utf-8"),
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bot6 Multi-Asset Scanner — NSE Equity DTC Strategy",
    ...
    parser.add_argument(
        "--strategy",
        default="Bot6_Scanner",
        help="Strategy name for OpenAlgo order tagging (default: Bot6_Scanner).",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
...
def run_backtest_mode(args: argparse.Namespace) -> None:
    ...
    from bot6.report import generate_report, export_trades_csv
    from bot6.config import WATCHLIST, TOTAL_CAPITAL

    symbols = args.symbols.split(",") if args.symbols else WATCHLIST
    symbols = [s.strip().upper() for s in symbols]

    log.info("=" * 60)
    log.info("  BOT6 SCANNER — BACKTEST MODE")
...
def run_live_mode(args: argparse.Namespace) -> None:
    from bot6.live_engine import run_live
    ...
    log.info("=" * 60)
    log.info("  BOT6 SCANNER — LIVE MODE")
