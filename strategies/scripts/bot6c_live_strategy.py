...
# Add the root openalgo directory to sys.path so we can import bot6c
...
from bot6c.position_manager import Position, Side, compute_quantity
    ...
    while True:
        try:
            ...
            if _is_after(now, SQUARE_OFF_TIME):
                if position and position.is_open:
                    ...
                    response = client.placesmartorder(
                        strategy="Bot6c_DTC", symbol=SYMBOL, action="SELL" if position.side == Side.LONG else "BUY",
            ...
            if latest_action_buy and not latest_action_sell:
                ...
                response = client.placesmartorder(
                    strategy="Bot6c_DTC", symbol=SYMBOL, action="BUY",
                    exchange=EXCHANGE, price_type="MARKET", product="MIS",
                ...
                response = client.placesmartorder(
                    strategy="Bot6c_DTC", symbol=SYMBOL, action="SELL",
                    exchange=EXCHANGE, price_type="MARKET", product="MIS",
