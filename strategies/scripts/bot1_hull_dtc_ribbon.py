...
EXECUTION_MODE = os.getenv("EXECUTION_MODE", "options_spread").lower()
        ...
        try:
            ...
            if past_square_off:
                if active_spread:
                    ...
                    close_position(client, active_spread)
                ...
                if signal:
                    ...
                    if signal in ("EXIT_LONG", "EXIT_SHORT"):
                        ...
                        if not expiry_formatted:
                            ...
                            active_spread = execute_trade(client, spot_price, signal, expiry_formatted, LOT_SIZE * LOT_MULTIPLIER, EXECUTION_MODE)
                            ...
                            if active_spread:
                                ...
                                # Subscribe to instruments based on profile
                                client.connect()
                                if active_spread.get("profile") == "options_spread":
                                    client.subscribe_ltp([
                                        {"exchange": OPTION_EXCHANGE, "symbol": active_spread["atm_symbol"]},
                                        {"exchange": OPTION_EXCHANGE, "symbol": active_spread["otm_symbol"]}
                                    ])
                                    ...
                                    exchange_used = OPTION_EXCHANGE # Fyers handles futures in NFO usually
                                    client.subscribe_ltp([
                                        {"exchange": exchange_used, "symbol": active_spread["symbol"]}
                                    ])
