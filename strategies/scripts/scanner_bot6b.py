...
==============================
Bot6b Multi-Asset Scanner — Main Entry Point.
...
LOGGING:
  Default: INFO level to stdout + bot6b_scanner.log file
...
# ---------------------------------------------------------------------------
# Path setup: add openalgo root so `bot6b` can be imported
# ---------------------------------------------------------------------------
...
def setup_logging(debug: bool = False) -> None:
    ...
    handlers = [
        ...
        logging.FileHandler("bot6b_scanner.log", mode="a", encoding="utf-8"),
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bot6b Multi-Asset Scanner — NSE Equity DTC Strategy",
    ...
    parser.add_argument(
        "--strategy",
        default="Bot6b_Scanner",
        help="Strategy name for OpenAlgo order tagging (default: Bot6b_Scanner).",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
...
def run_backtest_mode(args: argparse.Namespace) -> None:
    from bot6b.backtest_engine import run_backtest
    from bot6b.report import generate_report, export_trades_csv
    from bot6b.config import WATCHLIST, TOTAL_CAPITAL

    symbols = args.symbols.split(",") if args.symbols else WATCHLIST
    symbols = [s.strip().upper() for s in symbols]

    log.info("=" * 60)
    log.info("  BOT6b SCANNER — BACKTEST MODE")
...
def run_live_mode(args: argparse.Namespace) -> None:
    from bot6b.live_engine import run_live
    from bot6b.config import WATCHLIST
    ...
    log.info("=" * 60)
    log.info("  BOT6b SCANNER — LIVE MODE")
