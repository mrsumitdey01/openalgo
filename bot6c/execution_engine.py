"""
bot6c/execution_engine.py
========================
Main event-driven execution loop for Bot6c DTC Intraday Strategy.
...
class Bot6cEngine:
    """
    Event-driven bar processor for Bot6c.

    Usage
    -----
    engine = Bot6cEngine(symbol="RELIANCE")
    ...
    def run(self, df: pd.DataFrame) -> list[dict]:
        ...
        log.info(f"Bot6c started for {self.symbol} | Bars: {len(df_work)}")

        # --- Step 2: Process bar by bar ---
        for ts, bar in df_work.iterrows():
            self._process_bar(ts, bar)

        log.info(f"Bot6c finished for {self.symbol} | Trades: {len(self.trades)}")
        return self.trades
