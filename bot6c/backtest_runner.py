"""
bot6c/backtest_runner.py
=======================
Backtesting harness for Bot6c DTC Intraday Strategy.
...
HOW TO RUN:
    1. Place your 1-min CSV data in the bot6c/ directory.
       Expected format:
...
from execution_engine import Bot6cEngine
...
def print_summary(summary: dict, symbol: str) -> None:
    ...
    sep = "=" * 60
    print(f"\n{sep}")
    print(f"  BOT6C DTC BACKTEST RESULTS - {symbol}")
    print(sep)
...
def run_backtest(symbol: str, df: pd.DataFrame) -> dict:
    """
    Run Bot6c backtest on a given OHLCV DataFrame.
    ...
    """
    engine = Bot6cEngine(symbol=symbol)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bot6c DTC Intraday Backtest Runner"
    )
    parser.add_argument(
