# Bot6 — DTC Intraday Stocks Execution Engine

## Overview
Bot6 is a production-grade intraday execution daemon for **NSE cash equities**, implementing the **DTC (Dynamic Trend Confirmation)** momentum strategy. It is a direct Python translation of the TradingView Pine Script 6-EMA ribbon indicator, built with strict **zero-repainting** and **zero-lookahead-bias** guarantees.
...
├── bot6_trades.log        # Execution log (auto-created on first run)
├── bot6_tradelog.csv      # Trade CSV log (auto-created on first run)
└── tests/
